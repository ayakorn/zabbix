...
export verbose=0
tmpinput=/var/tmp/zblp_sitetcpool.$(whoami).dat
...
do
    ...
    url=${url%/}    # remove last /
    url="$url/$site/monitoring?part=lastValue&graph=usedConnections"
    val=$(curl -s $url)
    if [ "$val" == "" ]
    then
        echo "$zbhost lesspaper.site.$site.tomcat.jdbcpool 0" >> $tmpinput
    else
        echo "$zbhost lesspaper.site.$site.tomcat.jdbcpool $val" >> $tmpinput
    fi
done
